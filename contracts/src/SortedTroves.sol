...
contract SortedTroves is ISortedTroves {
    ...
    function _descendOne(
        ...
        if (
            _pos.nextId == ROOT_NODE_ID || _annualInterestRate > _troveManager.getTroveAnnualInterestRate(_pos.nextId)
        ) {
            found = true;
        } else {
    ...
    function _ascendOne(
        ...
        if (
            _pos.prevId == ROOT_NODE_ID || _troveManager.getTroveAnnualInterestRate(_pos.prevId) >= _annualInterestRate
        ) {
            found = true;
        } else {
