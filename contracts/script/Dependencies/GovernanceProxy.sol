...
contract GovernanceProxy is IGovernance, IUserProxyFactory, IMultiDelegateCall {
    ...
    function votesForInitiativeSnapshot(
        address _initiative
    ) external view override returns (uint256 votes, uint256 forEpoch, uint256 lastCountedEpoch, uint256 vetos) {
    ...
    ) external override returns (uint256 lusdSent, uint256 ethSent) {
    ...
    function getInitiativeSnapshotAndState(
        address _initiative
    )
        external
        view
    ...
    function getInitiativeState(
        address _initiative
    ) external override returns (InitiativeStatus status, uint256 lastEpochClaim, uint256 claimableAmount) {
    ...
    function getInitiativeState(
        ...
        return
            governance.getInitiativeState(_initiative, _votesSnapshot, _votesForInitiativeSnapshot, _initiativeState);
    }
