...
contract TroveEventsTest is EventsTest, ITroveEvents {
    ...
    function test_LiquidateEmitsLiquidation() external {
        ...
        (uint256 liquidatedTroveId, uint256 liquidatedColl) = openTroveWithExactICRAndDebt(
            A,
            0,
            MCR,
            liquidatedDebt,
    ...
    function test_BatchLiquidateTrovesEmitsLiquidation() external {
        ...
        for (uint256 i = 0; i < liquidatedDebt.length; ++i) {
            (liquidatedTroveIds[i], liquidatedColl[i]) = openTroveWithExactICRAndDebt(
                A,
                i,
                MCR,
                liquidatedDebt[i],
        ...
        for (uint256 i = 0; i < liquidatedDebt.length; ++i) {
            ...
            l.collSurplus = collRemaining -= l.collRedistributed = Math.min(
                collRemaining,
                (l.debtRedistributed * (DECIMAL_PRECISION + LIQUIDATION_PENALTY_REDISTRIBUTION)) / price
            );
