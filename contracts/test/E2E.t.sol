...
IBorrowerOperationsV1 constant mainnet_V1_borrowerOperations = IBorrowerOperationsV1(
...
contract E2ETest is Test, UseDeployment {
    ...
    function _allocateLQTY_end() internal {
        governance.allocateLQTY(
            _allocateLQTY_initiativesToReset,
            _allocateLQTY_initiatives,
            _allocateLQTY_votes,
            _allocateLQTY_vetos
        );
    ...
    function _mainnet_V1_redeemCollateralFromTroveAtTail(
        address redeemer,
        uint256 lusdAmount
    ) internal returns (uint256 redemptionFee) {
        address lastTrove = mainnet_V1_sortedTroves.getLast();
    ...
    function test_E2E() external {
        ...
        if (block.chainid == 1) {
            ...
            assertNotEq(
                ...
                address(0),
                "Mainnet should have DeFi Collective initiative"
            );
        }
        ...
        address liquidityProvider = makeAddr("liquidityProvider");
        {
            ...
            if (address(curveUsdcBoldGauge) != address(0)) {
                _depositIntoCurveGauge(
                    liquidityProvider,
                    curveUsdcBoldGauge,
                    curveUsdcBold.balanceOf(liquidityProvider)
                );
            }
            ...
            if (address(curveLusdBoldGauge) != address(0)) {
                _depositIntoCurveGauge(
                    liquidityProvider,
                    curveLusdBoldGauge,
                    curveLusdBold.balanceOf(liquidityProvider)
                );
            }
        }
        ...
        address staker = makeAddr("staker");
        {
            ...
            assertApproxEqAbsDecimal(
                lusd.balanceOf(staker),
                (lusdAmount * lqtyStake) / totalLQTYStaked,
                1e5,
                18,
                "LUSD reward"
            );
