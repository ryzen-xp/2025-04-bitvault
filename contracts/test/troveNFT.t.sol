...
contract troveNFTTest is DevTestSetup {
    ...
    function openMulticollateralTroveNoHints100pctWithIndex(
        ...
        uint256 avgInterestRate = contractsArray[_collIndex].activePool.getNewApproxAvgInterestRateFromTroveChange(
            troveChange
        );
    ...
    function testTroveURIAttributes() public view {
        ...
        assertTrue(
            LibString.contains(
                decodedUri,
                '"description": "Liquity V2 is a collateralized debt platform. Users can lock up'
            ),
            "NFT description attribute missing"
        );

        assertTrue(
            LibString.contains(decodedUri, '"trait_type": "Collateral Token"'),
            "Collateral Token attribute missing"
        );
        assertTrue(
            LibString.contains(decodedUri, '"trait_type": "Collateral Amount"'),
            "Collateral Amount attribute missing"
        );
        ...
        assertTrue(
            LibString.contains(decodedUri, '"value": "10000000000000000000"'),
