...
contract TestDeployer is MetadataDeployment {
    ...
    function _deployAndConnectCollateralContractsDev(
        ...
        addresses.metadataNFT = getAddress(
            address(this),
            getBytecode(type(MetadataNFT).creationCode, address(initializedFixedAssetReader)),
            SALT
        );
        ...
        addresses.stabilityPool = getAddress(
            address(this),
            getBytecode(type(StabilityPool).creationCode, address(contracts.addressesRegistry)),
            SALT
        );
    ...
    function deployAndConnectContractsMainnet(
        ...
        (vars.addressesRegistries[0], troveManagerAddress) = _deployAddressesRegistryMainnet(
        ...
        (vars.addressesRegistries[1], troveManagerAddress) = _deployAddressesRegistryMainnet(
        ...
        (vars.addressesRegistries[2], troveManagerAddress) = _deployAddressesRegistryMainnet(
    ...
    function _deployLeverageZappers(
        ...
        zappers.leverageZapperCurve = _deployCurveLeverageZapper(
            _addressesRegistry,
            _flashLoanProvider,
            _curveExchange,
            _lst
