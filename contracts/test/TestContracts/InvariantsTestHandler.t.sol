...
import {_100pct, _1pct, COLL_GAS_COMPENSATION_CAP, COLL_GAS_COMPENSATION_DIVISOR, DECIMAL_PRECISION, ETH_GAS_COMPENSATION, INITIAL_BASE_RATE, INTEREST_RATE_ADJ_COOLDOWN, MAX_ANNUAL_BATCH_MANAGEMENT_FEE, MAX_ANNUAL_INTEREST_RATE, MIN_ANNUAL_INTEREST_RATE, MIN_ANNUAL_INTEREST_RATE, MIN_BOLD_IN_SP, MIN_DEBT, MIN_INTEREST_RATE_CHANGE_PERIOD, ONE_MINUTE, ONE_YEAR, REDEMPTION_BETA, REDEMPTION_FEE_FLOOR, REDEMPTION_MINUTE_DECAY_FACTOR, UPFRONT_INTEREST_PERIOD, URGENT_REDEMPTION_BONUS} from "src/Dependencies/Constants.sol";
...
contract InvariantsTestHandler is Assertions, BaseHandler, BaseMultiCollateralTest {
            ...
            for (uint256 i = 0; i < _troveIds[j].size(); ++i) {
                ...
                if (batchManagementFee > 0) {
                    assertNotEq(
                        batchManager,
                        address(0),
                        "Trove accruing batch management fee should have batch manager"
                    );
                }
    ...
    function _openTrove(OpenTroveContext memory v) internal {
        ...
        try _functionCaller.call(address(v.c.borrowerOperations), _encodeOpenTrove(v)) {
            ...
            if (selector == BorrowerOperations.InvalidInterestBatchManager.selector) {
                ...
                assertFalse(
                    _batchManagers[v.i].has(v.batchManager),
                    "Shouldn't have failed as batch manager was valid"
                );
    ...
    function adjustTrove(
            ...
            )
        {
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertGtDecimal(
                    -v.collDelta,
                    int256(v.t.entireColl),
                    18,
                    "Shouldn't have failed as withdrawal <= coll"
                );
            } else if (selector == BorrowerOperations.DebtBelowMin.selector) {
                ...
                assertNotEq(
                    v.batchManager,
                    address(0),
                    "Shouldn't have thrown ICRBelowMCRPlusBCR as Trove wasn't in a batch"
                );
                ...
                assertLtDecimal(
                    ...
                    -v.$collDelta36,
                    36,
                    "Shouldn't have failed as repayment >= withdrawal"
                );
            } else {
                revert(string.concat("Unexpected error: ", v.errorString));
            }
    ...
    function adjustTroveInterestRate(
        ...
        v.wasActive = _isActive(i, v.troveId);
        v.premature =
            newInterestRate != v.t.annualInterestRate &&
        ...
        vm.prank(msg.sender);
        try
            v.c.borrowerOperations.adjustTroveInterestRate(
                v.troveId,
                newInterestRate,
                v.upperHint,
                v.lowerHint,
    ...
    function redeemCollateral(uint256 amount, uint256 maxIterationsPerCollateral) external {
        ...
        (uint256 totalDebtRedeemed, mapping(uint256 branchIdx => RedemptionTransientState) storage r) = _planRedemption(
            amount,
        ...
        for (uint256 j = 0; j < branches.length; ++j) {
            ...
            for (uint256 i = 0; i < r[j].batchManagers.size(); ++i) {
                batchManagementFee[j][i] = branches[j]
                    .troveManager
                    .getLatestBatchData(r[j].batchManagers.get(i))
    ...
    function provideToSP(uint256 i, uint256 amount, bool claim) external {
        ...
        try v.c.stabilityPool.provideToSP(amount, claim) {
            ...
            uint256 newBoldYield = v.totalBoldDeposits >= DECIMAL_PRECISION
                ? (v.blockedSPYield * v.boldDeposit) / v.totalBoldDeposits
                : 0;

            assertApproxEqAbsDecimal(
                v.c.stabilityPool.getCompoundedBoldDeposit(msg.sender),
                v.boldDeposit,
                1e7,
                18,
                "Wrong deposit"
    ...
    function registerBatchManager(
        ...
        minInterestRateChangePeriod = _bound(
            ...
            )
        {
            ...
            assertFalse(isShutdown[i], "Should have failed as branch had been shut down");
            assertFalse(
                _batchManagers[i].has(msg.sender),
                "Should have failed as batch manager had already registered"
            );
            ...
            assertLeDecimal(
                ...
                18,
                "Should have failed as fee > max"
            );
            assertGe(
                minInterestRateChangePeriod,
                MIN_INTEREST_RATE_CHANGE_PERIOD,
                "Should have failed as period < min"
            );
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertTrue(
                    _batchManagers[i].has(msg.sender),
                    "Shouldn't have failed as batch manager hadn't registered yet"
                );
            } else if (selector == BorrowerOperations.InterestRateTooLow.selector) {
                assertTrue(
                ...
                assertGtDecimal(
                    ...
                    MAX_ANNUAL_BATCH_MANAGEMENT_FEE,
                    18,
                    "Shouldn't have failed as fee <= max"
                );
    ...
    function lowerBatchManagementFee(uint256 i, uint256 newManagementFee) external {
        ...
        try c.borrowerOperations.lowerBatchManagementFee(newManagementFee) {
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertGeDecimal(
                    newManagementFee,
                    batch.managementRate,
                    18,
                    "Shouldn't have failed as new fee was lower"
                );
            } else {
                revert(string.concat("Unexpected error: ", errorString));
            }
    ...
    function setInterestBatchManager(
        ...
        vm.prank(msg.sender);
        try
            v.c.borrowerOperations.setInterestBatchManager(
                v.troveId,
                v.newBatchManager,
                v.upperHint,
                v.lowerHint,
            ...
            )
        {
            ...
            if (selector == BorrowerOperations.IsShutDown.selector) {
                ...
                assertNotEq(
                    _batchManagerOf[i][v.troveId],
                    address(0),
                    "Shouldn't have failed as Trove wasn't in a batch"
                );
            } else if (selector == BorrowerOperations.InvalidInterestBatchManager.selector) {
                assertFalse(
                    _batchManagers[i].has(v.newBatchManager),
                    "Shouldn't have failed as batch manager was valid"
                );
            } else if (selector == BorrowerOperations.ICRBelowMCRPlusBCR.selector) {
                uint256 newICR = _ICR(i, 0, 0, v.upfrontFee, v.t);
    ...
    function setBatchManagerAnnualInterestRate(
        ...
        newAnnualInterestRate = _bound(
            ...
            batch.interestRateMin > 0 ? batch.interestRateMin - 1 : 0,
            batch.interestRateMax + 1
        );
        v.upperHint = _pickHint(i, upperHintSeed);
        v.lowerHint = _pickHint(i, lowerHintSeed);
        ...
        v.premature =
            newAnnualInterestRate != batch.interestRate &&
        ...
        vm.prank(msg.sender);
        try
            v.c.borrowerOperations.setBatchManagerAnnualInterestRate(
            ...
            )
        {
            ...
            assertGeDecimal(
                newAnnualInterestRate,
                batch.interestRateMin,
                18,
                "Should have failed as interest rate wasn't in range"
            );
            assertLeDecimal(
                newAnnualInterestRate,
                batch.interestRateMax,
                18,
                "Should have failed as interest rate wasn't in range"
            );
            assertGe(
            ...
            if (v.upfrontFee > 0) {
                ...
                if (batchDebt == 0) {
                    assertApproxEqAbsDecimal(
                        v.b.entireDebtWithoutRedistribution,
                        0,
                        1e3,
                        18,
                        "Batch debt should be negligible"
                    );
    ...
    function _planOneRedemption(
        uint256 i,
        uint256 troveId,
        uint256 remainingAmount,
        uint256 feePct
    ) internal returns (uint256 debtRedeemed) {
    ...
    function _planRedemption(
        ...
        uint256 feePct
    ) internal returns (uint256 totalDebtRedeemed, mapping(uint256 branchIdx => RedemptionTransientState) storage r) {
    ...
    function _planUrgentRedemption(
        uint256 i,
        uint256 amount
    ) internal returns (UrgentRedemptionTransientState storage r) {
        r = _urgentRedemption;
    ...
    function _decodeCustomError(
        bytes memory revertData
    ) public pure returns (bytes4 selector, string memory errorString) {
