...
contract LiquidationsTest is DevTestSetup {
    ...
    function testLiquidationOffsetWithSurplus() public {
        ...
        uint256 collSurplusAmount = (collAmount * 995) /
            1000 -
            ((((liquidationAmount + initialValues.AInterest) * DECIMAL_PRECISION) / price) * 105) /
            100;
        assertApproxEqAbs(
    ...
    function testLiquidationMix() public {
        ...
        assertEq(
            vars.spBoldBalance - finalSPBoldBalance,
            vars.liquidationAmount / 2 - 1e18,
            "SP Bold balance mismatch"
        );
        // Check SP Coll has  increased
        uint256 finalSPCollBalance = stabilityPool.getCollBalance();
        ...
        uint256 collSurplusAmount = (vars.collAmount * 995) /
            1000 -
            // Portion offset
            ((((vars.liquidationAmount / 2 - 1e18) * DECIMAL_PRECISION) / vars.price) * 105) /
            100 -
            // Portion redistributed
            ((((vars.liquidationAmount / 2 + 1e18 + vars.AInterest) * DECIMAL_PRECISION) / vars.price) * 110) /
            100;
        assertApproxEqAbs(
            collToken.balanceOf(address(collSurplusPool)),
            collSurplusAmount,
        ...
        vm.stopPrank();
        assertApproxEqAbs(
            collToken.balanceOf(A) - vars.ACollBalance,
            collSurplusAmount,
            10,
            "A collateral balance mismatch"
        );
