...
contract MockTroveManager {
    ...
    function getTroveAnnualInterestRate(TroveId troveId) public view returns (uint256) {
        return
            _troves[troveId].batchId.isZero()
    ...
    function _sortedTroves_insert(TroveId id, uint256 annualInterestRate, Hints memory hints) external {
        ...
        _sortedTroves.insert(
            TroveId.unwrap(id),
            annualInterestRate,
            TroveId.unwrap(hints.prev),
            TroveId.unwrap(hints.next)
        );
    }
    ...
    function _sortedTroves_findInsertPosition(
        uint256 annualInterestRate,
        Hints memory hints
    ) external view returns (Hints memory) {
    ...
    function _sortedTroves_validInsertPosition(
        uint256 annualInterestRate,
        Hints memory hints
    ) external view returns (bool) {
        return
            _sortedTroves.validInsertPosition(
                annualInterestRate,
                TroveId.unwrap(hints.prev),
                TroveId.unwrap(hints.next)
            );
...
contract SortedTrovesTest is Test {
        ...
        for (uint256 i = 0; i < troves.length; ++i) {
            ...
            if (role == ArbRole.Individual) {
                ...
                tm._sortedTroves_insertIntoBatch(
                    troveId,
                    batchId,
                    tm.getTroveAnnualInterestRate(troveId),
                    _pickHints(troves[i].hints)
                );
            } else {
    ...
    function test_SortsBatchedTrovesByAnnualInterestRate(
        ...
        for (uint256 i = 0; i < reInsertions.length; ++i) {
            ...
            if (batchId.isNotZero()) {
                tm._sortedTroves_reInsertBatch(
                    batchId,
                    reInsertions[i].newAnnualInterestRate,
                    _pickHints(reInsertions[i].hints)
                );
