...
contract ZapperLeverageMainnet is DevTestSetup {
    ...
    function setUp() public override {
        ...
        TestDeployer.TroveManagerParams[] memory troveManagerParamsArray = new TestDeployer.TroveManagerParams[](
            NUM_COLLATERALS
        );
        ...
        TestDeployer.DeploymentResultMainnet memory result = deployer.deployAndConnectContractsMainnet(
    ...
    function fundUniV3Pools(TestDeployer.LiquityContracts[] memory _contractsArray) internal {
        for (uint256 i = 0; i < NUM_COLLATERALS; i++) {
            ...
            // mint new position
            address uniV3PoolAddress = uniswapV3Factory.getPool(
                address(boldToken),
                address(_contractsArray[i].collToken),
                UNIV3_FEE
            );
    ...
    function openLeveragedTroveWithIndex(
        OpenLeveragedTroveWithIndexParams memory _inputParams
        ...
        vars.flashLoanAmount =
            (_inputParams.collAmount * (_inputParams.leverageRatio - DECIMAL_PRECISION)) /
            DECIMAL_PRECISION;
    ...
    function _setInitialBalances(ILeverageZapper _leverageZapper, uint256 _branch, TestVars memory vars) internal view {
        ...
        vars.collBalanceBeforeExchange = contractsArray[_branch].collToken.balanceOf(
            address(_leverageZapper.exchange())
        );
    }
    ...
    function _testCanOpenTrove(
        ...
        assertEq(
            boldToken.balanceOf(address(_leverageZapper)),
            vars.boldBalanceBeforeZapper,
            "Zapper should not keep BOLD"
        );
        assertEq(
        ...
        assertEq(
            address(_leverageZapper.exchange()).balance,
            vars.ethBalanceBeforeExchange,
            "Exchange should not keep ETH"
        );
        if (_branch > 0) {
            // LST
    ...
    function _getLeverUpFlashLoanAndBoldAmount(LeverUpParams memory _params) internal returns (uint256, uint256) {
        ...
        vars.flashLoanAmount =
            (vars.currentCollAmount * _params.leverageRatio) /
            vars.currentLR -
            vars.currentCollAmount;
        vars.expectedBoldAmount = (vars.flashLoanAmount * vars.price) / DECIMAL_PRECISION;
    ...
    function testOnlyOwnerOrManagerCanLeverUpWithHybridFromBalancerFLProvider() external {
        ...
        for (uint256 i = 0; i < 3; i++) {
            _testOnlyOwnerOrManagerCanLeverUpFromBalancerFLProvider(
                leverageZapperHybridArray[i],
                ExchangeType.HybridCurveUniV3,
                i
            );
        }
    }
    ...
    function _testCanLeverDownTrove(
        ILeverageZapper _leverageZapper,
        ExchangeType _exchangeType,
        uint256 _branch
    ) internal {
        ...
        // debt
        uint256 expectedMinNetDebt = vars.initialDebt -
            (((vars.flashLoanAmount * vars.price) / DECIMAL_PRECISION) * 101) /
            100;
        uint256 expectedMaxNetDebt = (expectedMinNetDebt * 105) / 100;
        uint256 troveEntireDebt = getTroveEntireDebt(contractsArray[_branch].troveManager, vars.troveId);
        assertGe(troveEntireDebt, expectedMinNetDebt, "Debt too low");
        ...
        uint256 ICR = contractsArray[_branch].troveManager.getCurrentICR(vars.troveId, vars.price);
        assertTrue(
            ICR >= vars.resultingCollateralRatio || vars.resultingCollateralRatio - ICR < CRTolerance,
            "Wrong CR"
        );
        // token balances
        assertEq(boldToken.balanceOf(A), vars.boldBalanceBeforeA, "BOLD bal mismatch");
    ...
    function testOnlyOwnerOrManagerCanLeverDownWithHybridFromZapper() external {
        // Not enough liquidity for ETHx
        for (uint256 i = 0; i < 3; i++) {
            _testOnlyOwnerOrManagerCanLeverDownFromZapper(
                leverageZapperUniV3Array[i],
                ExchangeType.HybridCurveUniV3,
                i
            );
        }
    ...
    function _getCloseFlashLoanAmount(
        uint256 _troveId,
        ITroveManager _troveManager,
        IPriceFeed _priceFeed
    ) internal returns (uint256) {
    ...
    function _testCanCloseTrove(IZapper _zapper, uint256 _branch) internal {
        ...
        uint256 debtInColl = (getTroveEntireDebt(contractsArray[_branch].troveManager, troveId) * DECIMAL_PRECISION) /
            price;
        ...
        assertLe(
            ...
            (boldBalanceBefore * 105) / 100,
            "BOLD bal can only increase by slippage margin"
        );
        if (lst) {
            ...
            assertApproxEqAbs(
                A.balance,
                ethBalanceBefore + collAmount + ETH_GAS_COMPENSATION - debtInColl,
                3e17,
                "ETH bal mismatch"
            );
        }
    ...
    function _testApprovalIsNotReset(
        ILeverageZapper _leverageZapper,
        ExchangeType _exchangeType,
        uint256 _branch
    ) internal {
    ...
    function _getBoldAmountToSwapUniV3(
        uint256 _maxBoldAmount,
        uint256 _minCollAmount,
        IERC20 _collToken /* view */
    ) internal returns (uint256) {
    ...
    function _getBoldAmountToSwapHybrid(
        uint256 _maxBoldAmount,
        uint256 _minCollAmount,
        IERC20 _collToken /* view */
    ) internal returns (uint256) {
