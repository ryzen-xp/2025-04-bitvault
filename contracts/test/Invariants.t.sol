...
contract InvariantsTest is Assertions, Logging, BaseInvariantTest, BaseMultiCollateralTest {
    ...
    function invariant_SystemStateMatchesGhostState() external view {
        for (uint256 i = 0; i < branches.length; ++i) {
            ...
            assertApproxEqAbsDecimal(
                c.pools.collSurplusPool.getCollBalance(),
                handler.collSurplus(i),
                10,
                18,
                "Wrong CollSurplusPool"
            );
            assertApproxEqAbsDecimal(
            ...
            assertApproxEqAbsDecimal(
                c.stabilityPool.getCollBalance(),
                handler.spColl(i),
                10,
                18,
                "Wrong StabilityPool coll"
            );
            ...
            for (uint256 j = 0; j < handler.numTroves(i); ++j) {
                ...
                assertApproxEqAbsDecimal(
                    t.entireColl,
                    coll,
                    Math.max(totalCollRedist / 1e6, 1e5),
                    18,
                    "Wrong Trove coll"
                );
                assertApproxEqAbsDecimal(
                    t.entireDebt,
                    debt,
                    Math.max(totalDebtRedist / 1e6, 1e5),
                    18,
                    "Wrong Trove debt"
                );
                ...
                assertEq(
                    c.borrowerOperations.interestBatchManagerOf(troveId),
                    batchManager,
                    "Wrong batch manager (BO)"
                );

                if (status == ITroveManager.Status.active) {
                    assertEq(
                        BatchId.unwrap(c.sortedTroves.getBatchOf(troveId)),
                        batchManager,
                        "Wrong batch manager (ST)"
                    );
                }
            }
        ...
        assertEqDecimal(
            collateralRegistry.getRedemptionRateWithDecay(),
            handler.getRedemptionRate(),
            18,
    ...
    function invariant_StabilityPool_AllBoldClaimable_ExceptYieldReceivedWhileEmpty() external view {
        for (uint256 j = 0; j < branches.length; ++j) {
            ...
            assertLt(
                stabilityPool.getTotalBoldDeposits() - sumBoldDeposit,
                1000,
